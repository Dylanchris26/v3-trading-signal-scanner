...
V3 experimental practice/demo signal scanner.

- Practice/demo only
- Auto-trading OFF
- Maximum 12 signals/day
...
def analyze_setup(
    ...
    """Pure V3 strategy evaluation. No network/live freshness checks.

    V3 is deliberately less dependent on a single exact EMA-reclaim pattern.
    It accepts two entry families:
      1) trend pullback/reclaim, or
      2) strong two-candle continuation after a controlled retracement.
    Higher-timeframe trend alignment remains mandatory.
    """
    ...
    # V3 uses wider but still directional RSI zones to avoid killing signal count.
    if direction == "CALL" and not (52.0 <= rsi_value <= 68.0):
        return None
    if direction == "PUT" and not (32.0 <= rsi_value <= 48.0):
        return None
    ...
    continuation_ok = two_candle_confirmation(candles_1m, direction)
    mom = candle_momentum(candles_1m)
    strong_momentum = (direction == "CALL" and mom >= 0.16) or (direction == "PUT" and mom <= -0.16)

    # At least one concrete entry trigger is required.
    if not pullback_ok and not (continuation_ok and strong_momentum):
        return None

    # Keep a modest amount of room before the nearest opposing level.
    if not room_to_opposite_level_v3(candles_1m, direction, atr_value):
        return None
    ...
    reasons.append("5M and 15M trend agree with direction.")
    ...
    if continuation_ok:
        score += 6
        confirmations += 1
        reasons.append("Two-candle continuation confirms direction.")
    if strong_momentum:
        score += 5
        confirmations += 1
        reasons.append("Recent 1M momentum confirms direction.")

    if direction == "CALL" and 55 <= rsi_value <= 63:
        score += 5
        confirmations += 1
        reasons.append("RSI is in the preferred CALL zone.")
    elif direction == "PUT" and 37 <= rsi_value <= 45:
        score += 5
        confirmations += 1
    ...
    if score < (MIN_SCORE_CALL if direction == "CALL" else MIN_SCORE_PUT) or confidence < MIN_CONFIDENCE:
        return None
...
def room_to_opposite_level_v3(candles: Sequence[Candle], direction: str, atr_value: float) -> bool:
    """V3 uses a smaller 0.35 ATR clearance than V2's stricter 0.55 ATR."""
    support, resistance = support_resistance(candles, lookback=60)
    price = candles[-1].close
    room = 0.35 * atr_value
    if direction == "CALL":
