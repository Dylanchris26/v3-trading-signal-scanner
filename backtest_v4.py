"""
IQ Option Strategy V3 Backtester
...
import scanner_v4 as strategy

PAIRS = strategy.REGULAR_PAIRS
CANDLES_PER_REQUEST = 1000
HISTORY_MINUTES = int(os.getenv("BACKTEST_MINUTES", "14400"))  # 10-day validation window
MAX_SIGNALS_PER_PAIR = int(os.getenv("BACKTEST_MAX_SIGNALS_PER_PAIR", "60"))
MIN_GAP_MINUTES = int(os.getenv("BACKTEST_SIGNAL_GAP", "3"))
END_OFFSET_MINUTES = int(os.getenv("BACKTEST_END_OFFSET_MINUTES", "14400"))  # finish at the configured historical offset
...
def fetch_history(
    ...
    end = server_time(iq) if end_timestamp is None else int(end_timestamp)
...
def run_pair(
    ...
    end_timestamp = server_time(iq) - END_OFFSET_MINUTES * 60
    c1 = fetch_history(iq, symbol, 60, HISTORY_MINUTES, end_timestamp)
    c5 = fetch_history(iq, symbol, 300, HISTORY_MINUTES, end_timestamp)
    c15 = fetch_history(iq, symbol, 900, HISTORY_MINUTES, end_timestamp)
    ...
    final_index = len(c1) - strategy.V4_EXPIRY_MINUTES - 1
...
def print_report(results: List[Result]) -> None:
    print("\n" + "=" * 68)
    print("IQ OPTION V4 STRATEGY BACKTEST REPORT")
    print("=" * 68)
    ...
    if not results:
        print("No qualifying V4 historical signals were found.")
        print("The V4 gates may simply be too strict for this sample.")
        return
    ...
    print("\nScore bands:")
    for low, high in ((78, 79), (80, 84), (85, 89), (90, 100)):
    ...
    print("- This run uses an unseen historical window ending before the original V3 test window.")
...
def main() -> None:
    ...
    try:
        ...
        for pair in PAIRS:
            symbol = pair
            market = "REGULAR"
            test_end = server_time(iq) - END_OFFSET_MINUTES * 60
            test = fetch_history(iq, symbol, 60, 180, test_end)
            ...
            try:
                ...
                if results:
                    ...
                    print("  Result: no qualifying V3 signals")
