...
# V6.2: no 15-second candle-age requirement.
...
def _flag_is_false(value):
    ...
    return str(value).strip().lower() in {
        "false", "0", "closed", "disabled", "off", "no"
    }
...
def _flag_is_true(value):
    ...
    if isinstance(value, (int, float)):
        return value == 1
    return str(value).strip().lower() in {
        "true", "1", "suspended", "closed", "disabled", "off", "no"
    }


def normalize_broker_name(name):
    # Handles EURUSD, EURUSD-op, 1.EURUSD-op and similar broker formats.
    name = str(name or "").strip().upper()

    if "." in name:
        name = name.split(".")[-1].strip()

    for suffix in ("-OP", "-BINARY", "-TURBO"):
...
def load_binary_status(iq):
    # V6.2: resilient broker-status parsing. The known Digital crash path
    # (get_all_open_time) is intentionally not used.
    for attempt in range(1, 4):
        ...
        try:
            ...
            if not isinstance(result, dict):
                print(f"Broker status attempt {attempt}: response was not a dictionary.")
            else:
                for mode in ("turbo", "binary"):
                    ...
                    for key, detail in actives.items():
                        ...
                        if isinstance(detail, dict):
                            for field in ("name", "active_name", "symbol", "underlying"):
                                value = detail.get(field)
                                if value:
                                    raw_names.insert(0, value)

                        normalized = None
                        for raw_name in raw_names:
                            candidate = normalize_broker_name(raw_name)
                            normalized_seen.add(candidate)
                            if candidate in REGULAR_SET or candidate in OTC_SET:
                        ...
                        if isinstance(detail, dict):
                            if "enabled" in detail and _flag_is_false(detail.get("enabled")):
                                continue
                            if "suspended" in detail and _flag_is_true(detail.get("suspended")):
                                continue

                        if normalized in REGULAR_SET:
                            regular_open.add(normalized)
                        elif normalized in OTC_SET:
                            otc_open.add(normalized)

            print(
                f"Broker status attempt {attempt}: "
                f"{len(regular_open)} regular open, {len(otc_open)} OTC open."
            )

            if normalized_seen:
                print(
                    "Broker normalized sample:",
                    ", ".join(sorted(normalized_seen)[:20])
                )

            if regular_open or otc_open:
                print(
            ...
            print(f"Broker status attempt {attempt} error:", repr(e))

        if attempt < 3:
            time.sleep(2)

    print("Broker status unavailable after 3 attempts. No signal will be generated.")
    return set(), set()
...
def scan_pair(iq, pair, state):
    ...
    # Critical V6.2 protection: never issue an already-started entry.
