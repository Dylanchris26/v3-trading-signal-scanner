...
def load_binary_status(iq):
    """Load broker binary/turbo status once, without using the Digital status path."""
    try:
...
def broker_binary_open(detail, pair):
    ...
    for mode_name in ("turbo", "binary"):
        ...
        # API versions have returned both booleans and numeric flags.
        if bool(enabled) and not bool(suspended):
            return True
    return False
...
def main():
    ...
    for pair in REGULAR_PAIRS:
        if broker_binary_open(broker_detail, pair):
            df = candles(iq, pair)
            if fresh(df):
