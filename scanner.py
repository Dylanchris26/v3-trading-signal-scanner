...
def latest_closed_candle(df):
    if df is None or len(df) < 3:
        return None
    now = time.time()
    closed = df[(df["ts"] + 60) <= now + 0.5]
    if closed.empty:
        return None
...
def fresh(df):
    c = latest_closed_candle(df)
    if c is None:
        return False
    age = time.time() - (float(c["ts"]) + 60.0)
...
def main():
    ...
    # The broker initialization contains many assets (stocks, indices,
    # crypto, commodities, etc.) that the installed API cannot request
    # candle data for. Restrict the scanner to supported forex pairs.
    supported_regular = {
        "EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD",
        "NZDUSD", "EURJPY", "GBPJPY", "EURGBP", "EURCAD", "AUDJPY",
    ...
    for raw_name in broker_detail:
        # IQ Option commonly exposes regular forex as EURUSD-op while
        # candle requests use EURUSD.
        normalized = raw_name[:-3] if raw_name.endswith("-op") else raw_name

        if normalized in supported_regular and broker_binary_open(broker_detail, raw_name):
            regular_open.append(normalized)
        elif normalized in supported_otc and broker_binary_open(broker_detail, raw_name):
            otc_open.append(normalized)

    regular_open = sorted(set(regular_open))
    otc_open = sorted(set(otc_open))
    ...
    for market, pair, df in candidates:
        closed = df[df["ts"] + 60 <= time.time() + 0.5].copy()
        if len(closed) < 60 or not fresh(closed):
            continue
        ...
        ranked.append({
            ...
            "entry_ts": float(closed["ts"].iloc[-1]) + 60.0,
    ...
    ranked = [z for z in ranked if z["entry_ts"] - time.time() >= MIN_ENTRY_LEAD_SECONDS]
    if not ranked:
        save_state(state)
        print("All setups are too late for a fresh 1-minute entry. No signal.")
        return
