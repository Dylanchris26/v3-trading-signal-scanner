...
def broker_binary_open(iq, pair):
    # get_binary_option_detail() is based on broker initialization data and
    # reports binary/turbo details without entering the Digital status path.
    try:
        detail = iq.get_binary_option_detail()
        item = detail.get(pair)
        if not item:
            return False
        for mode_name in ("turbo", "binary"):
            info = item.get(mode_name)
            if isinstance(info, dict):
                if info.get("enabled") is True and info.get("is_suspended") is not True:
                    return True
        return False
    except Exception as e:
        print("Broker availability check failed for", pair, ":", e)
        return False
...
def main():
    ...
    # Regular market first.
    candidates = []
    for pair in REGULAR_PAIRS:
        if broker_binary_open(iq, pair):
            df = candles(iq, pair)
            if fresh(df):
                candidates.append(("REGULAR", pair, df))

    # OTC only if no regular candidate is broker-reported open/fresh.
    if not candidates:
