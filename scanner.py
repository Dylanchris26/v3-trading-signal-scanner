...
def connect_iq() -> IQ_Option:
    ...
    # Refresh IQ Option's active-name -> active-id map. This is needed for
    # OTC symbols that are not present in the package's old constants.py.
    try:
        iq.update_ACTIVES_OPCODE()
        LOGGER.info("IQ Option active symbols refreshed.")
    except Exception as exc:
        LOGGER.warning("Could not refresh IQ Option active symbols: %s", exc)
...
def _discover_otc_symbols(iq: IQ_Option, regular_pair: str) -> List[str]:
    """Discover exact OTC symbols from IQ Option's binary/turbo payload."""
